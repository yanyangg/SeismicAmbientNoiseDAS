...
#%% parameters
path_preprocessed = '/net/jin/ssd-tmp-nobak2/yyang7/MorroBay_preprocess/'
output_CC = '/net/jin/ssd-tmp-nobak2/yyang7/MorroBay_CC/'
if not os.path.exists(output_CC):
    os.mkdir(output_CC)

# starting and ending dates to compute daily stacked CC
dates = [x.strftime('%Y%m%d') for x in pd.date_range(start='12/09/2022', end='12/09/2022', freq='1D')]

# Now give the No. of channel pairs to calculate CC.
# below is an example of common-shot
nch = 10000
pair_channel1 = 2500 * np.ones(nch, dtype='int')
...
for idate in tqdm(dates):
    ...
    for ifile in filelist:
        ...
        for ichunk in range(nchunk):
            ...
            # t1 = time.time()
            cc = cross_correlation(data1, data2, is_spectral_whitening = True, whitening_params=whitening_params).cpu().numpy()
...
ccf = filter(ccall,50, .1, 4)
vmax = np.percentile(np.abs(ccf), 50)
